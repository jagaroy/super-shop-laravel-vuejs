<template>
    <div class="row">
      ...
      <div class="col-md-2 grid-margin stretch-card">
        <button @click="exportjsPdf()" class="btn btn-sm btn-info">Category List PDF</button>
      </div>

      <div class="col-md-2 grid-margin stretch-card">
        <button @click="downloadXLSX()" class="btn btn-sm btn-info">Category List XLSX</button>
      </div>
          ...
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-md-4 align-self-end">
                <input autocomplete="off" type="text" @keyup="CategoryList(1, true)" class="form-control form-control-sm fw-bold"
                  placeholder="Search..." id="search_button" />
              </div>
              <div class="col-md-1 align-self-center">
                <button class="btn btn-sm btn-success" @click="CategoryList(1, true)">Search</button>
              </div>
            </div>
            <div id="temp-target">
                ...
                <table id="table" class="table " style="width: 100%;">
                  ...
                  <tbody>
                    ...
                    <tr v-for="(data, index) in CategoryListData.data" :key="index">
                      ...
                      <td>{{ data.remarks }}</td>
                      <td>
                        <button @click="CategoryEdit(data.id)" class="btn btn-success btn-sm">Edit</button>
                        <button @click="deleteCategory(data.id)" class="btn btn-danger btn-sm ms-1">Delete</button>
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <nav aria-label="..." class="mt-2">
              <ul class="pagination justify-content-center">
                <li v-for="(link, indexing) in CategoryListData.links" :key="indexing" class="page-item">
                  <a class="page-link" href="#" :class="link.active ? 'active' : ''" @click="CategoryList(link.label)"
                    v-html="link.label">
    ...
    <!-- Modal -->
    <div class="modal fade bd-example-modal-lg" id="addCategoryModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form class="row g-3" id="addForm" @submit.prevent="submitCategory" enctype="multipart/form-data">
    ...
    <!-- Edit Category Modal -->
    <div class="modal fade bd-example-modal-lg" id="editCategoryModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header bg-info">
          ...
          <div class="modal-body">
            <form class="row g-3" id="editForm" @submit.prevent="submitEditCategory(form.id)" enctype="multipart/form-data">
    ...
    setup() {
      ...
      const submitCategory = async (e) => {
        ...
        .then(res => {
          ...
          .catch(err => {
            console.log("🚀 ~ submitCategory ~ err:", err)
      ...
      const CategoryList = async (page = 1, search = false) => {
        ...
        await axios.get(`/api/admin/categories?page=${page}&search=${query}`)
          .then(res => {
      ...
      const CategoryEdit = async (id) => {
        await axios.get('/api/admin/categories/' + id + '/edit')
          .then(res => {
            if (res.data.status == 'success') {
      ...
      const submitEditCategory = async (id) => {
        ...
        await axios.post('/api/admin/category_update/' + id, form, config)
          .then(res => {
            console.log(res)
            if (res.data.status == 'success') {
      ...
      function exportjsPdf() {
        ...
        var y = 10;
        doc.setLineWidth(2);
        doc.text(200, y = y + 20, "Category List report");
        ...
        doc.save("Category-table-" + now_mls + ".pdf");
      }
      ...
      function downloadXLSX() {
        ...
        let now_mls = Date.now();
        XLSX.writeFile(wb, 'Category List Report-' + now_mls + '.xlsx')
      }
